...
- (void)jsq_configureMessagesViewController
{
    ...
    self.inputToolbar.contentView.textView.placeHolder = [NSBundle jsq_localizedStringForKey:@"Type a message..."];
...
- (UICollectionViewCell *)collectionView:(JSQMessagesCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!isMediaMessage) {
        ...
        @weakify(messageMedia);
        
        if ([messageMedia mediaView] == nil && [messageMedia isKindOfClass:[JSQLocationMediaItem class]]) {
            [((JSQLocationMediaItem*)messageMedia) setLocation:((JSQLocationMediaItem*)messageMedia).location withCompletionHandler:^{
                BSDispatchBlockToMainQueue(^{
                    @strongify(messageMedia);
                    if ([messageMedia mediaView])
    ...
    BOOL needsAvatar = !isOutgoingMessage;
...
//    }
    ...
    if (needsAvatar) {
        avatarImageDataSource = [collectionView.dataSource collectionView:collectionView avatarImageDataForItemAtIndexPath:indexPath];
            ...
            NSString *avatarImage = (NSString*)avatarImageDataSource;
            if (avatarImage == nil) {
                cell.avatarImageView.image = [[FRUserManager sharedInstance] avatarPlaceholder];
                cell.avatarImageView.highlightedImage = nil;
            }
            else {
                [cell.avatarImageView sd_setImageWithURL:[NSURL URLWithString:avatarImage] placeholderImage:[FRUserManager sharedInstance].avatarPlaceholder];
//
            }
    }
    ...
    if (isOutgoingMessage) {
        cell.messageBubbleTopLabel.textInsets = UIEdgeInsetsMake(0.0f, 8.0f, 0.0f, bubbleTopLabelInset);
    }
    else {
...
- (void)keyboardController:(JSQMessagesKeyboardController *)keyboardController keyboardDidChangeFrame:(CGRect)keyboardFrame
{
    ...
    CGFloat heightFromBottom = CGRectGetMaxY(self.collectionView.frame) - CGRectGetMinY(keyboardFrame) != 0 ? CGRectGetMaxY(self.collectionView.frame) - CGRectGetMinY(keyboardFrame) : self.heightForAd ;
