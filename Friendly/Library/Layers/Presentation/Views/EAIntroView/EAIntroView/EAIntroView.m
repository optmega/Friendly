...
- (UIImageView *)pageBgBack {
    if (!_pageBgBack) {
        ...
        _pageBgBack.backgroundColor = [UIColor redColor];
...
- (UIImageView *)pageBgFront {
    if (!_pageBgFront) {
        ...
        _pageBgFront.backgroundColor = [UIColor redColor];
...
//        [pageView addConstraints:constraints];
        [page.customView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)scrollViewDidScroll:(EARestrictedScrollView *)scrollView {
    [self.delegate scroll:scrollView.contentOffset.x];
