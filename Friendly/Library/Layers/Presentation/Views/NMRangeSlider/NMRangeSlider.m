...
- (void) configureView
{
    ...
    self.tintColor = [UIColor bs_colorWithHexString:kPurpleColor];//@"7340F2"];
}
...
- (UIImage *)trackBackgroundImage
{
    if(_trackBackgroundImage==nil)
    {
        if(IS_PRE_IOS7())
        {
            UIImage* image = [FRStyleKit imageOfSlider];//[self imageFromBundle:@"slider-default-trackBackground"];
            image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(0.0, 5.0, 0.0, 5.0)];
            _trackBackgroundImage = image;
        }
        else
        {
            UIImage *image = [FRStyleKit imageOfSlider];
...
- (void) addSubviews
{
    ...
    self.trackBackground.backgroundColor = [UIColor bs_colorWithHexString:@"E8E6F1"];
